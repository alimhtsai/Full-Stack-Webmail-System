...
export function createState(inParentComponent: React.Component): any {
  ...
  if (stateSingleton === null) {
    ...
    stateSingleton = {
       ...
       * @param inCategory The Email category of the contact to show.
       */
      showContact : function(inID: string, inName: string, inEmail: string, inCategory: string): void {

        console.log("state.showContact()", inID, inName, inEmail, inCategory);

        this.setState(() => ({
          currentView : "contact", contactID : inID, contactName : inName, contactEmail : inEmail, contactCategory : inCategory
        }));
      ...
      showAddContact : function(): void {
        ...
        this.setState(() => ({
          currentView : "contactAdd", contactID : null, contactName : "", contactEmail : "", contactCategory : "",
        }));
      ...
      saveContact : async function(): Promise<void> {
        ...
        console.log("state.saveContact()", this.state.contactID, this.state.contactName, this.state.contactEmail, this.state.contactCategory);
        ...
        const contact: Contacts.IContact =
          await contactsWorker.addContact({ name : this.state.contactName, email : this.state.contactEmail, category : this.state.contactCategory });
        this.state.showHidePleaseWait(false);
        ...
        // Update state.
        this.setState(() => ({ contacts : cl, contactID : null, contactName : "", contactEmail : "", contactCategory : "" }));
       ...
       */
       updateContact : async function(): Promise<void> {
        ...
        console.log("state.updateContact()", this.state.contactID, this.state.contactName, this.state.contactEmail, this.state.contactCategory);
        ...
        this.state.showHidePleaseWait(true);
        const contactsWorker: Contacts.Worker = new Contacts.Worker();
        console.log("-----" + this.state.contactCategory);
        const contact: Contacts.IContact =
          await contactsWorker.updateContact({ _id: this.state.contactID, name: this.state.contactName, email: this.state.contactEmail, category: this.state.contactCategory });
        this.state.showHidePleaseWait(false);

        // Set update name and update email of the updated contact
        for (let i = 0; i < cl.length; i++ ) {
          if (cl[i]._id == this.state.contactID) {
            cl[i].name = this.state.contactName;
            cl[i].email = this.state.contactEmail;
            cl[i].category = this.state.contactCategory;
          }
        }
